...
fn release_bots(
    ...
    for (trans, mut swarm_point) in query_swarm_point.iter_mut() {
        ...
        for _ in 0..count {
            ...
            let scene = SceneRoot(scene_assets.spaceship.clone());
            let entity = commands
                .spawn((
                    bot,
                    transform,
                ...
                SwarmBotState::Swarming => {
                    ...
                    if let Some((e2, l_trans, b2)) = leader_transform {
                        ...
                        if (l_trans.translation - t1.translation).length() <= b1.swarm_spacing_max {
                            ...
                            info!("{} b2 thrust", b2.thrust);
                            b1.thrust = b2.thrust;
                        } else {
                            let diff = l_trans.translation - t1.translation;
                            b1.target_dir = Dir3::new(diff.normalize_or(Vec3::ZERO)).unwrap();
                            info!("{} swarming with {}", e1.to_bits(), e2.to_bits());

                            b1.thrust = b2.thrust + 0.1 * diff.length();
